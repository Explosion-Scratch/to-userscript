(function () {
  ...
  // #endregion
  // #region Abstraction Layer Vanilla Target
			        ...
			        request.onupgradeneeded = (event) => {
			          ...
			          try {
			            if (!db.objectStoreNames.contains(STORAGE_STORE_NAME)) {
			              ...
			              console.log(
			                "Created IndexedDB object store:",
			                STORAGE_STORE_NAME,
			              );
			            }
			          } catch (storeError) {
			  ...
			  async function _fetch(url, options = {}) {
			    try {
			      ...
			      if (options && typeof options !== "object") {
			        throw new Error(
			          `Fetch options must be an object, got ${typeof options}`,
			        );
			      }
			  ...
			  function _getMimeTypeFromPath() {
			    return "application/octet-stream";
			  }
			  function _isTextAsset() {
			    return false;
			  }
			  function _createAssetUrl() {
			    return "";
			  }
  ...
  // #region Orchestration Logic
	  const SCRIPT_NAME = "Cloud To Butt Plus";
  ...
  // #region Script Execution Logic
		  async function executeAllScripts(globalThis, extensionCssData) {
		    ...
		    const scriptName = "Cloud To Butt Plus";
  ...
  // #endregion
    // #region Document End
			    if (typeof document !== "undefined") {
			      ...
			      const scriptPaths = ["content_script.js"];
			      console.log(`  Executing JS (end): ${scriptPaths}`);
			
			      try {
			        // Keep variables from being redeclared for global scope, but also make them apply to global scope. (Theoretically)
			        with (globalThis) {
			          // START: content_script.js
			          walk(document.body);
			          ...
			          function handleText(textNode) {
			            ...
			            if (v.match(/cloud/i)) {
			              ...
			              if (
			                v.match(
			                  /PaaS|SaaS|IaaS|computing|data|storage|cluster|distributed|server|hosting|provider|grid|enterprise|provision|apps|hardware|software|/i,
			                )
			              ) {
			                v = v.replace(
			                  /(C|c)loud/gi,
